...
class LocalDB {
  static async saveProfile(email, password, data) {
    ...
    try {
      const a = await AsyncStorage.setItem("email", email);
      const b = await AsyncStorage.setItem("password", password);
      const c = await AsyncStorage.setItem("profile", JSON.stringify(data));
      console.log("asdsdsdsdfdfdjdjjdjdjdjdjddjjdjd", a, b, c);
      return true;
    } catch (error) {
