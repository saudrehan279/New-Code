  ...
  UNSAFE_componentWillMount = async () => {
    ...
    if (response.success) {
      ...
      if (store.SEARCHING.LISTING_FILTER.data.is_rated_enabled) {
        store.SEARCHING.LISTING_FILTER.data.rated.option_dropdown.forEach(
          (item) => {
      ...
      store.SEARCHING.LISTING_FILTER.data.sorting.option_dropdown.forEach(
        (item) => {
          item.checkStatus = false;
        }
      );
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.subCon}>
          {data.has_admob && data.admob.banner !== "" ? (
            <AdMobBanner
              adSize={
                Platform.OS === "ios" ? "smartBanner" : "smartBannerLandscape"
              }
              adUnitID={data.admob.banner} // 'ca-app-pub-3940256099942544/6300978111'
              onAdFailedToLoad={async () =>
                await this.setState({ loadAdd: false })
              }
              didFailToReceiveAdWithError={async () =>
                await this.setState({ loadAdd: false })
              }
              onAdLoaded={async () => {
                await this.setState({ loadAdd: true });
              }}
            />
          ) : null}
          {/* <BannerView
          ...
          >
            <View style={styles.topViewCon}>
              <View style={styles.InnerRadius}>
                <View style={styles.imageCon}>
                  <ImageBackground
                    indicator={null}
                    source={{ uri: home.search_section.image }}
                  ...
                  >
                    ...
                    >
                      <View style={styles.findTxtCon}>
                        <Text style={styles.firTxt}>
                          {home.search_section.main_title}
                        </Text>
                      </View>
                      <Text style={styles.secTxt}>
                        {home.search_section.sub_title}
                      </Text>
                      <View style={styles.searchCon}>
                        <TextInput
                          ...
                          style={[
                            styles.txtInput,
                            I18nManager.isRTL ? { textAlign: "right" } : null,
                          ]}
                        />
                        <Icon
                          size={30}
            ...
            {home.categories_enabled ? (
              ...
              >
                  ...
                  <FlatList
                    ...
                    renderItem={({ item, key }) => (
                      ...
                      >
                        ...
                        <Text style={[styles.childTxt, { fontWeight: "bold" }]}>
                          {item.name}
                        </Text>
                      </TouchableOpacity>
                    )}
                ...
                <View style={{ flex: 1.3, width: width(100) }} />
              </View>
            ) : null}
            {home.listings_enabled ? (
              ...
              >
                ...
                <TouchableOpacity
                  style={[
                    styles.readMoreBtnCon,
                    { borderColor: store.settings.data.navbar_clr },
                  ]}
                  onPress={() =>
                    this.navigateToScreen(
                      "SearchingScreen",
                ...
                >
                  <Text style={[styles.latestFeature, { fontSize: 13 }]}>
                    {home.section_btn}
                  </Text>
                </TouchableOpacity>
              </View>
            ...
            home.featured_listings.has_featured_listings ? (
              <View
                style={{
                  width: width(100),
                  marginTop: 10,
                  ...
                  {home.featured_listings.list.map((item, key) => {
                    return (
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          (store.LIST_ID = item.listing_id),
                            this.props.navigation.navigate(
                              "FeatureDetailTabBar",
                              {
                                listId: item.listing_id,
                                list_title: item.listing_title,
                              }
                            );
                        }}
                      >
                    ...
                    return (
                      <TouchableOpacity
                        style={{
                          height: height(16),
                          width: width(43),
                          marginRight: this.isOdd(key) ? width(3) : 0,
                          marginVertical: 5,
                          alignItems: "center",
                        }}
                        onPress={() => {
            ...
            {home.events_enabled ? (
              <View style={styles.cate_con}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => this.navigateToScreen("PublicEvents", "Home")}
                >
                  <Text style={[styles.latestFeature, { fontSize: 13 }]}>
                    {home.view_all_events}
                  </Text>
                </TouchableOpacity>
              </View>
            ) : null}
          ...
          <TouchableOpacity
            style={[styles.exploreBtn, { backgroundColor: data.main_clr }]}
            onPress={() => this.navigateToScreen("SearchingScreen", "Advance")}
          >
