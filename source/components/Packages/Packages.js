...
// import {
//   Platform, StyleSheet, Text, View, Image,Linking, I18nManager, ScrollView, TouchableOpacity, Picker, ActivityIndicator, RefreshControl
// } from 'react-native';
...
//     // try {
//     //   console.warn(code);
//     //   RNIap.requestPurchase(code,true); //ios: com.scriptsbundle.MomBoss.business , android: 12 ,item.android.code
...
//                     </TouchableOpacity>
//                 } */}
//                 <TouchableOpacity style={{ marginVertical: 0, width: width(70), borderRadius: 5, justifyContent: 'center', alignItems: 'center', backgroundColor: main_clr, opacity: item.already_purchased ? 0.5 : 1 }}
//                   onPress={() => {
//                     if(item.already_purchased){

//                     }else{
//                       Linking.openURL('https://mombossmarket.space/packages/')
//                     }
//                     // this.inAppPurchase(item.package_id, item.package_type, 'free', item)
//                   }}>
//                   <Text style={{ marginVertical: 15, fontSize: 16, color: COLOR_PRIMARY }}>{item.already_purchased ? data.user_btn : 'Purchase'}</Text>
//                 </TouchableOpacity>
//               </View>
//           }
//         </View>
...
export default class Packages extends Component<Props> {
    ...
    if (products.length > 0) {
      this.purchaseUpdateSubscription = RNIap.purchaseUpdatedListener(
        (purchase: ProductPurchase) => {
          ...
          if (Platform.OS === "ios") {
            RNIap.finishTransactionIOS(purchase.transactionId);
          } else RNIap.consumePurchaseAndroid(purchase.purchaseToken);
  ...
  getPackages = async () => {
    ...
    try {
      var response = await ApiController.post("packages");
      console.log("packages vvvvvvvvvvvvvvvvvvvv are====>>>", response);

      if (response.success) {
        store.PACKAGES_OBJ = response.data;
        store.PACKAGES_OBJ.packages.forEach((item) => {
          item.selectedMethod = "";
          if (item.ios.code.length != 0) itemSkewsIos.push(item.ios.code);
          if (item.android.code.length != 0)
            itemSkewsAndroid.push(item.android.code);
        });
  ...
  freePackage = async (pkg_id, pkg_type, type) => {
    console.log("pakaaaaaaaaaaaaaaaaaaaaaaaaa", pkg_id, pkg_id, type);
    this.setState({ loading: true });
    if (type === "free") {
      var params = {
        package_id: pkg_id,
    ...
    console.log(
      ...
      ApiController.post("payment", params)
    );
    if (response.success) {
      store.settings.data.package = response.data.package;
      // await this.getPackages();
  ...
  _blog = (item, key) => {
    ...
    return (
      <View key={key} style={styles.blogCon}>
        ...
        >
          <Text style={{ marginVertical: 20, color: "#6E768B", fontSize: 22 }}>
            {item.package_info.title}
          </Text>
        </View>
        <View
          style={{
        ...
        >
          <Text style={{ marginVertical: 20, color: "#6E768B", fontSize: 30 }}>
            <Text style={{ fontSize: 14 }}>{item.package_info.symbol}</Text>
            {item.package_info.price}
          </Text>
        </View>
        <View
          style={{
            ...
            {"expiry" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.expiry.title}: {item.expiry.value}
                </Text>
              </View>
            ) : null}
            {"listings" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.listings.title}: {item.listings.value}
                </Text>
              </View>
            ) : null}
            {"listings_expiry" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.listings_expiry.title}: {item.listings_expiry.value}
                </Text>
              </View>
            ) : null}
            {"featured" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.featured.title}: {item.featured.value}
                </Text>
              </View>
            ) : null}
            {"featured_expiry" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.featured_expiry.title}: {item.featured_expiry.value}
                </Text>
              </View>
            ) : null}
            {"video" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.video.title}: {item.video.value}
                </Text>
              </View>
            ) : null}
            {"weblink" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.weblink.title}: {item.weblink.value}
                </Text>
              </View>
            ) : null}
            {"images" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.images.title}: {item.images.value}
                </Text>
              </View>
            ) : null}
            {"price_range" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.price_range.title}: {item.price_range.value}
                </Text>
              </View>
            ) : null}
            {"business_hours" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.business_hours.title}: {item.business_hours.value}
                </Text>
              </View>
            ) : null}
            {"tags" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.tags.title}: {item.tags.value}
                </Text>
              </View>
            ) : null}
            {"bump" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.bump.title}: {item.bump.value}
                </Text>
              </View>
            ) : null}
            {"coupon" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.coupon.title}: {item.coupon.value}
                </Text>
              </View>
            ) : null}
            {"event" in item ? (
              <View key={key} style={styles.stripCon}>
                ...
                <Text style={styles.text}>
                  {item.event.title}: {item.event.value}
                </Text>
              </View>
            ) : null}
          </View>
          ...
          {item.package_type === "free" ? (
            <TouchableOpacity
              style={{
                ...
                opacity: item.already_purchased ? 0.5 : 1,
              }}
              onPress={() => {
                if (!item.already_purchased) {
                  this.freePackage(item.package_id, item.package_type, "free");
            ...
            >
              ...
              >
                {item.already_purchased ? data.user_btn : data.btn_txt}
              </Text>
            </TouchableOpacity>
          ) : (
            <View
            ...
            >
              {Platform.OS === "android" ? (
                (console.log(Platform.OS, "gggggdddddddddd"),
                (
                  <View
                    style={{
                      height: height(6),
                  ...
                  >
                    <Picker
                      ...
                      onValueChange={async (itemValue, itemIndex) => {
                        // await this.setState({ payment_methodes: item.payments_methods, pkg_id: item.package_id, pkg_type: item.package_type, amount: item.package_info.price, currency: item.package_info.currency }),
                        await this.callerPaymentMethodes(
                          itemValue,
                          item.package_id,
                          item.package_type,
                          item.package_info.price,
                          item.package_info.currency,
                          item
                        );
                      }}
                    >
              ...
              >
                ...
                >
                  {item.already_purchased ? data.user_btn : "Purchase"}
                </Text>
              </TouchableOpacity> */}
            </View>
          )}
        ...
        {this.state.loading ? (
          ...
          >
            {data.has_package ? (
              ...
              >
                <Image
                  source={require("../../images/profileWarning.png")}
                  style={{
                    height: height(7),
                    width: width(15),
                    resizeMode: "contain",
                ...
                />
                <Text
                  style={{ fontSize: totalSize(2), color: COLOR_SECONDARY }}
                >
                  {data.message}
                </Text>
              </View>
          ...
          >
            <View style={{ height: height(4), alignItems: "flex-end" }}>
              <TouchableOpacity
                ...
                onPress={() => {
                  this.setState({ is_picker: false });
                }}
              >
                <Image
                  source={require("../../images/clear-button.png")}
                  style={{
                    height: height(2),
                    width: width(3),
                    resizeMode: "contain",
